...
public class GatewayConfig {
    ...
    public RouterFunction<ServerResponse> publicNotificationRoutes() {
        return GatewayRouterFunctions.route("public-notification-service")
            .route(
                RequestPredicates.path("/notifications/**").and(
    ...
    @Bean
    public RouterFunction<ServerResponse> protectedPaymentRoutes() {
        return GatewayRouterFunctions.route("payment-service")
            .route(
                RequestPredicates.path("/create-payment-intent").or(
                    RequestPredicates.path("/payments/**")
                ),
                HandlerFunctions.http(URI.create("http://payment-service:1234"))
            )
            .filter(authenticationFilter.apply()) // Protect all payment routes
            .build();
    }
}
